...
struct DynamicConvOpV2ToV1 : public OpRewritePattern<DynamicConvOpV2> {
  ...
  LogicalResult matchAndRewrite(DynamicConvOpV2 op,
    ...
    // int64_t rank =
    // cast<mlir::vhlo::RankedTensorV1Type>(op.getLhs().getType())
    //                    .getShape().size();
    auto shape =
        cast<mlir::vhlo::RankedTensorV1Type>(op.getDPadding().getType()).getShape();
    int64_t size = std::accumulate(shape.begin(), shape.end(), 1,
                                   std::multiplies<int64_t>());
    // op.getLhs().cast<mlir::vhlo::RankedTensorV1Type>().getShape().size();
