...
TensorV1Attr getDefaultPadding(OpBuilder& builder, Value lhs) {
  ...
  int64_t numSpatialDimensions = lhsType.getShape().size() - 2;

  // Convert to DenseElements for getRawData handling.
  auto denseElements = DenseIntElementsAttr::get(
      RankedTensorType::get({numSpatialDimensions, 2}, builder.getI64Type()),
      SmallVector<int64_t>(numSpatialDimensions * 2, 0ll));

  return TensorV1Attr::get(
      builder.getContext(),
      RankedTensorV1Type::get(builder.getContext(), {numSpatialDimensions, 2},
