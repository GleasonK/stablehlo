...
LogicalResult inferDynamicConvOp(
  ...
  if (dPaddingType.getNumElements() > 0 &&
      (dPaddingShape[0] != numDims - 2 || dPaddingShape[1] != 2))
    return emitOptionalError(location, "expects d_padding to be of shape [",
                             numDims - 2, ", 2], but got [", dPaddingShape,
                             "]");

  if (SmallVector<int64_t> shape; succeeded(matchInts(dPadding, shape))) {
    SmallVector<ShapedTypeComponents> inferredReturnShapes;
    // auto paddingAttr =
    //     DenseIntElementsAttr::get(cast<ShapedType>(dPadding.getType()),
    //     shape);
    ...
    // TODO: add missing tests for DynamicConvOp.
    ...
    // std::string str;
    // llvm::raw_string_ostream os(str);
    // llvm::interleaveComma(outputDimensions, os, [&](int64_t i) { os << i; });
    // llvm::report_fatal_error(StringRef("ADFDSFSFSDFSSDF" + str));
  }
...
LogicalResult verifyDynamicConvOp(
  ...
  //     cast<RankedTensorType>(lhsType).getRank() - 2, {0, 0});
  if (SmallVector<int64_t> padding; succeeded(matchInts(dPadding, padding))) {
    auto paddingType = RankedTensorType::get(
        cast<RankedTensorType>(dPadding.getType()).getShape(),
        IntegerType::get(location->getContext(), 64));
    ...
    //   item = {first, second};
    // }
    // auto a = std::to_string(shape[0]);
    // llvm::report_fatal_error(
    //     StringRef(a + ", size: " + std::to_string(shape.size())));
    // auto paddingAttr =
    //     DenseIntElementsAttr::get(cast<ShapedType>(dPadding.getType()),
    //     shape);
    if (failed(inferConvolutionOp(
            location, lhsType, rhsType, windowStrides, paddingAttr, lhsDilation,
            rhsDilation, windowReversal, inputBatchDimension,
            inputFeatureDimension, inputSpatialDimensions,
            kernelInputFeatureDimension, kernelOutputFeatureDimension,
            kernelSpatialDimensions, outputBatchDimension,
            outputFeatureDimension, outputSpatialDimensions, featureGroupCount,
            batchGroupCount, precisionConfig, inferredReturnShapes)))
      return failure();
