...
class FunctionBuilder : public RegionOpBuilder<func::FuncOp> {
 public:
  FunctionBuilder(ModuleBuilder& mb, std::string name,
      ...
      : RegionOpBuilder(mb, mb.getOpBuilder().create<func::FuncOp>(
                                loc.value_or(mb.getLoc()), name,
...
// FuncOp builder, return the name of the function after insertion into the
// parent module. This may not match `name` if there was a symbol name conflict.
StringRef Func(MlirBuilder& mb, StringRef name,
...
#include "third_party/stablehlo/google/builder/FuncBuilder.h.inc"
